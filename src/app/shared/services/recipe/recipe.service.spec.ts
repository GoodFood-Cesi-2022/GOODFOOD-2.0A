...
import { mockRecipe1, mockRecipe2, mockRecipeArray } from "../../mock/recipe.mock";
...
describe("RecipeService (with mocks)", () => {
  ...
  describe("get all recipes()", () => {
    ...
    it("shoud return ALL recipes from API (called once)", () => {
      service.getRecipes().subscribe({
        next: (values) => expect(values).withContext("should return all recipes").toEqual(mockRecipeArray),
        error: fail,
      });

      /* RecipeService should have made one request to GET recipes from expected URL */
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes?includes[]=pictures`);
  ...
  describe("get one recipes()", () => {
    ...
    it("shoud return ALL recipes from API (called once)", () => {
      service.getRecipe(mockRecipe1.id).subscribe({
        next: (values) => expect(values).withContext("should return all recipes").toEqual(mockRecipe1),
        error: fail,
      });

      /* RecipeService should have made one request to GET recipes from expected URL */
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes/${mockRecipe1.id}`);
      expect(req.request.method).toBe(_HttpRequest.GET);
      // Respond with the mock recipes
  ...
  describe("should call getRecipeType()", () => {
    ...
    it("should return new recipe-type", () => {
      ...
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes/types`);
      expect(req.request.method).toEqual(_HttpRequest.GET);
  ...
  describe("should call createRecipe()", () => {
    ...
    it("should return new recipe", () => {
      ...
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes`);
  ...
  describe("get ingredients recipe()", () => {
    ...
    it("shoud return ALL ingredients for a recipe", () => {
      service.getIngredientsByRecipeId(mockRecipe1.id).subscribe({
        next: (values) =>
          expect(values).withContext("shoud return ALL ingredients for a recipe").toEqual(mockIngredientArray),
        error: fail,
      });

      /* RecipeService should have made one request to GET recipes from expected URL */
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes/${mockRecipe1.id}/ingredients`);
      expect(req.request.method).toBe(_HttpRequest.GET);
      // Respond with the mock recipes
  ...
  describe("uploadPicture()", () => {
    ...
    it("shoud upload a picture", () => {
      service.uploadPicture(picture1).subscribe({
        next: (values) => expect(values).withContext("shoud upload a picture").toEqual({ message: Message.UPDATE }),
        error: fail,
      });

      /* RecipeService should have made one request to GET recipes from expected URL */
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/files`);
  ...
  describe("attachPictures()", () => {
    ...
    it("shoud attach a picture to a recipe", () => {
      service.attachPictures(mockRecipe1, mockRecipe1).subscribe({
        next: (values) => expect(values).withContext("shoud return success of attachment").toEqual(Message.UPDATE),
        error: fail,
      });

      /* RecipeService should have made one request to GET recipes from expected URL */
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/recipes/${mockRecipe1.id}/pictures`);
      const formData = new FormData();
