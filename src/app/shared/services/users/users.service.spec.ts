  ...
  describe("get all users", (): void => {
    ...
    it("should return users (called once)", () => {
      service.getUsers().subscribe({
        next: (values) => expect(values).withContext("should return all users").toEqual(mockUserArray),
        error: fail,
      });
    ...
    it("should be OK returning no users", () => {
      service.getUsers().subscribe({
        next: (values) => expect(values.length).withContext("should have empty users array").toEqual(0),
        error: fail,
      });
    ...
    it("should return users (called multiple times)", () => {
      ...
      service.getUsers().subscribe();
      service.getUsers().subscribe({
        next: (values) => expect(values).withContext("should return users").toEqual(mockUserArray),
        error: fail,
      });

      const requests = httpTestingController.match(`${environment.apiBaseUrl}/users`);
  ...
  describe("get one user", (): void => {
    ...
    it("should return users (called once)", () => {
      service.getUser(mockUser1.id).subscribe({
        next: (values) => expect(values).withContext("should return one users").toEqual(mockUser1),
        error: fail,
      });
  ...
  describe("get user role", (): void => {
    ...
    it("should return users (called once)", () => {
      service.getUserRole(mockUserAuth1.id).subscribe({
        next: (values) => expect(values).withContext("should return one users").toEqual(mockRoleArray),
        error: fail,
      });

      // UsersService should have made one request to GET users from URL
      const req = httpTestingController.expectOne(`${environment.apiBaseUrl}/users/${mockUser1.id}/roles`);
      expect(req.request.method).toEqual(_HttpRequest.GET);

      // Respond with the mock users
      req.flush(mockRoleArray);
    });
  });
    ...
    it("should update user and return it", (): void => {
      service.updateUser(mockUser1).subscribe({
        next: (data: string): void =>
          expect(data).withContext("should return the user").toEqual(Message.UPDATE_SUCCESS),
        error: fail,
      });
  ...
  describe("should call deleteUser()", (): void => {
    ...
    it("should delete user and return success message", (): void => {
      service.deleteUser(mockUser1.id).subscribe({
        next: (data: string): void =>
          expect(data).withContext("should return the success message").toEqual(Message.DELETE),
        error: fail,
      });
