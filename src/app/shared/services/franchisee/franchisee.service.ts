...
export class FranchiseeService {
   ...
   */
	public getFranchisees(): Observable<Franchisee[]> {
		return this.http.get<Franchisee[]>(`${environment.apiBaseUrl}/contractors`).pipe(
			map((res: any) => res),
			tap((franchisees: any) => {
				this.log('fetched franchisees');
				console.log('service -> all franchisees : ', franchisees);
			}),
   ...
   */
	public getFranchiseeRecipes(recipe: FranchiseeRecipe): Observable<FranchiseeRecipe[]> {
		return this.http
			.get<FranchiseeRecipe[]>(`${environment.apiBaseUrl}/contractors/${recipe.id}/recipes`)
			.pipe(tap((obj: any) => console.log('service -> All recipes : ', obj)), map((res: any) => res['data']));
	}
   ...
   */
	public newFranchisee(create: Franchisee): Observable<Franchisee> {
		...
		return this.http
			...
			.pipe(tap((obj: any) => console.log('service -> All recipes : ', obj)), map((res: any) => res));
	}
   ...
   */
	public updateRecipePrice(update: Partial<FranchiseeRecipe>, recipe: Recipe): Observable<string> {
		return this.http
			.put(`${environment.apiBaseUrl}/contractors/${update.id}/recipes/${recipe.id}`, update)
			.pipe(
 ...
 */
	public removeRecipe(id: number, recipe: Recipe): Observable<string> {
		return this.http
			...
			.pipe(
				tap((obj: any) => console.log('service -> Remove recipe from catalog : ', obj)),
   ...
   */
	public getSchedule(franchisee: Franchisee): Observable<Schedule> {
		return this.http
			.get<Schedule>(`${environment.apiBaseUrl}/contractors/${franchisee.id}/times`)
			.pipe(tap((obj: any) => console.log('service -> schedule : ', obj)), map((res: any) => res));
	}
